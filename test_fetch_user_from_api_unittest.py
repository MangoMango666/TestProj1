...
class TestThisShit(unittest.TestCase):
    ...
    @unittest.skip(' ce test est ignoré grâce au décorateur')
    def testIgnoré(self): # exemple de test ignoré (temporairement) grâce au décorateur
        raise Exception()


class TestThisOtherShit(unittest.TestCase):
    ...
    def testOkWithMock(self): # test avec un user qui n'est obtenu avec une fausse requête http (simulée)
        ...
        us = createUserFromAPIUrl('https://randomuser.me/api')
        assert isinstance(us, User)
        self.assertEqual(us.name,'mr balder fjelldal')
...
class TestMock(unittest.TestCase):
    ''' Autre manière de faire les tests, en utilisant directement la librairie unittest.mock,
    elle-même utilisée par responses '''

    @mock.patch('fetch_user_from_api.createUserFromAPIUrl') # format nom_fichier_python.(classe).méthode_appelée
    def testMockUser(self,mocker_create_user): # mocker_create_user est l'alias que l'on utilise pour la méthode définie avec @mock.patch
        ...
        mocker_create_user.return_value = User.createFromJson(FAKE_USER) # affectation de la méthode qui simule
    ...
    # Remarque : problème avec l'environnement d'exécution sous PyCharm, ne fonctionne pas
    # mais fonctionne avec appel en ligne de commande : python -m unittest fetch_user_from_api.py
